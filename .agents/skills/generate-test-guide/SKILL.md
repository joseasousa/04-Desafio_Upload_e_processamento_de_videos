...
- Check if `.Codex/rules/` already has testing-related rules.
...
Sub-files are **not loaded automatically** — Codex reads them only when the SKILL.md's instructions direct it to, based on the artifact type being worked on.
...
- [ ] No contradiction with existing skills in `.Codex/skills/` or rules in `.Codex/rules/`
...
- **Do not modify** any existing files in `.Codex/rules/`.
